...
public class CatalogManager {
    ...
    public static void show_index() throws Exception {
        Index tmpIndex;
        Iterator<Map.Entry<String, Index>> iter = indexes.entrySet().iterator();
        ...
        String str = String.format(format,"INDEX", "TABLE", "ATTRIBUTE");
        ...
        while (iter.hasNext()) {
            ...
            str = String.format(format,tmpIndex.indexName, tmpIndex.tableName, tmpIndex.attributeName);
            Connect.out_to_client(str);
        }
    ...
    //打印内存中table的信息
    public static void show_table() throws Exception {
        Table tmpTable;
        ...
        while (iter.hasNext()) {
            ...
            Connect.out_to_client("[TABLE] " + tmpTable.tableName);

            String format = "|%-" + get_max_attr_length(tmpTable) + "s";
            format += "|%-5s|%-6s|%-6s|\n";
            String str = String.format(format,"ATTRIBUTE", "TYPE", "LENGTH", "UNIQUE");
 //         System.out.printf(format, "ATTRIBUTE", "TYPE", "LENGTH", "UNIQUE");
            System.out.printf(str);
            Connect.out_to_client(str);
            for (int i = 0; i < tmpTable.attributeNum; i++) {
                tmpAttribute = tmpTable.attributeVector.get(i);
                str = String.format(format,tmpAttribute.attributeName, tmpAttribute.type.get_type(), tmpAttribute.type.get_length(), tmpAttribute.isUnique);
